...
if (request_method() === 'POST' && verify_csrf(post('csrf_token'))) {
    ...
    } elseif ($action === 'rotate_share' && $user) {
...
<section class="py-16 sm:py-20">
  <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
    ...
    <p class="text-center text-brand-soft mb-10 max-w-xl mx-auto">Save pieces you love<?= $user ? ', then share your list with friends and family.' : ' — no account needed.' ?></p>

    <?php if (!$user): ?>
      <div class="max-w-2xl mx-auto mb-12 rounded-3xl border border-brand-ink/10 bg-white/80 p-5 sm:p-6 text-center">
        <p class="text-sm text-brand-ink/80 mb-3">Sign in to keep this list across devices and share it with friends.</p>
        <a href="<?= e(url('index.php?page=login')) ?>" class="text-sm underline text-brand-soft hover:text-brand-ink">Sign in</a>
      </div>
    <?php else: ?>
